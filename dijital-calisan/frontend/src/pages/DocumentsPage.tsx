...
export default function DocumentsPage() {
  ...
  useEffect(() => {
    void fetchDocs()
    managementApi.categories().then(r => setCategories(r.data)).catch(() => { })
  }, [fetchDocs])
  ...
  const handleCategoryChange = async (docId: string, categoryId: string) => {
    try {
      await documentApi.changeCategory(docId, categoryId || null)
      await fetchDocs()
    } catch (error) {
      setError(errorMessage(error, 'Kategori değiştirilemedi.'))
    }
  }
              ...
              <div key={doc.id} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400">
                    {CATEGORIES.find(item => item.value === doc.category)?.label} · {STATUS_LABELS[doc.status]} · {(doc.file_size_bytes / 1024).toFixed(0)} KB
                  </p>
                  {doc.processing_error_message && (
                    <p className="text-xs text-red-500 mt-0.5">{doc.processing_error_message}</p>
                  )}
                </div>
