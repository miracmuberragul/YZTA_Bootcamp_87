...
export default function DocumentsPage() {
  ...
  const fetchDocs = useCallback(async () => {
    ...
    setError('')
    try {
      const response = await documentApi.list({ status: statusFilter || undefined })
      ...
      <div className="bg-white rounded-xl border border-gray-100">
        <div className="flex items-center justify-between gap-4 px-4 py-3 border-b border-gray-100">
          ...
          <div className="flex items-center gap-2">
            <select value={statusFilter} onChange={event => setStatusFilter(event.target.value as DocumentStatus | '')} className="border border-gray-200 rounded-lg px-2 py-1.5 text-xs">
        ...
        {loading ? <div className="p-8 text-center text-gray-400 text-sm">Yükleniyor…</div> : docs.length === 0 ? (
          ...
          <div key={doc.id} className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50">
            ...
            {(doc.status === 'failed' || doc.status === 'uploaded') && <button onClick={() => void handleRetry(doc.id)} className="p-1.5 hover:bg-orange-50 rounded-lg" aria-label="Tekrar dene"><RefreshCw size={14} className="text-orange-500" /></button>}
            ...
            <button onClick={() => void handleDelete(doc)} className="p-1.5 hover:bg-red-50 rounded-lg" aria-label="Sil"><Trash2 size={14} className="text-red-400" /></button>
          </div>
        ))}</div>}
      </div>
    </div>
